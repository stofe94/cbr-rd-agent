...
from rdagent.core.conf import ExtendedBaseSettings, get_safe_cwd
...
class LLMSettings(ExtendedBaseSettings):
    ...
    show_llm_prompts: bool = True
    """
    Whether to show full LLM prompts and responses in terminal.
    When False, only shows "Model answer in progress..." while waiting for response.
    Backend-agnostic: works with both LiteLLM and Google AI Studio backends.
    """
    ...
    chat_auto_shrink_for_tpm: bool = False
    chat_allow_tpm_shrink: bool = False
    chat_tpm_limit: int | None = None
    chat_tpm_target_ratio: float = 0.85
    chat_tpm_strategy: Literal["wait", "shrink"] = "wait"
    chat_tpm_window_seconds: int = 60
    dump_chat_cache: bool = False
    use_chat_cache: bool = False
    dump_embedding_cache: bool = False
    use_embedding_cache: bool = False
    prompt_cache_path: str = str(get_safe_cwd() / "prompt_cache.db")
