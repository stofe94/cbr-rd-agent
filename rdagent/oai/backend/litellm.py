...
def _provider_env_prefix_for_model(model: str) -> str | None:
    try:
    ...
    if custom_llm_provider:
        if custom_llm_provider in _PROVIDER_TO_ENV_MAP:
            return _PROVIDER_TO_ENV_MAP[custom_llm_provider]
        return custom_llm_provider.upper()

    return "LITELLM" if os.environ.get("LITELLM_API_KEY") else None


def _api_key_for_model(model: str) -> str | None:
    env_prefix = _provider_env_prefix_for_model(model)
    if not env_prefix:
        return None
    # Intentionally use a single key only and do not rotate across *_API_KEY_N variables.
    return os.environ.get(f"{env_prefix}_API_KEY")
...
class LiteLLMAPIBackend(APIBackend):
    ...
    def _create_chat_completion_inner_function(  # type: ignore[no-untyped-def] # noqa: C901, PLR0912, PLR0915
        ...
        **kwargs,
        ...
        if LITELLM_SETTINGS.log_llm_chat_content:
            if LITELLM_SETTINGS.show_llm_prompts:
                logger.info(self._build_log_messages(messages), tag="llm_messages")
            else:
                logger.info(f"{LogColors.YELLOW}Model answer in progress...{LogColors.END}", tag="llm_messages")
        ...
        if LITELLM_SETTINGS.log_llm_chat_content and LITELLM_SETTINGS.show_llm_prompts:
