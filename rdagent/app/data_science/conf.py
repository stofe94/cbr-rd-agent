...
from rdagent.core.conf import get_safe_cwd
...
class DataScienceBasePropSetting(KaggleBasePropSetting):
    ...
    # Use DS_LOCAL_DATA_PATH from environment if set, else fallback to default.
    local_data_path: str | None = None

    @field_validator("local_data_path", mode="before")
    @classmethod
    def _normalize_local_data_path(cls, v: object) -> str:
        # If env provides empty value, fall back to the project default path.
        env_path = os.environ.get("DS_LOCAL_DATA_PATH")
        if v is None or (isinstance(v, str) and not v.strip()):
            if env_path:
                return env_path
            return str(get_safe_cwd() / "workspace")
        return str(v)
    ...
    kaggle_live_eval: bool = False
    """If true, evaluate by uploading submission.csv to Kaggle and reading back the returned score."""

    kaggle_live_eval_strict: bool = False
    """If true with kaggle_live_eval, never fallback to proxy/reference grading when live eval is unavailable."""

    kaggle_submission_poll_seconds: int = 20
    """Polling interval (seconds) while waiting for Kaggle submission results."""

    kaggle_submission_timeout_seconds: int = 900
    """Maximum wait time (seconds) for Kaggle submission scoring."""
    ...
    user_interaction_mid_folder: Path = (
        Path(local_data_path) / "RD-Agent_user_interaction"
        if local_data_path is not None else Path.cwd() / "workspace" / "RD-Agent_user_interaction"
    )


DS_RD_SETTING = DataScienceBasePropSetting()

def _ensure_local_data_root(path_str: str) -> Path:
    """Ensure a writable data root directory.
    
    Note: This function creates the base data directory without timestamping.
    Timestamping of run-specific workspaces is handled separately by the experiment code.
    """
    requested = Path(path_str).expanduser()
    
    try:
        # If any existing parent is a file, this candidate can never be a directory.
        ...
        if requested.is_dir():
            return requested
        raise NotADirectoryError(f"Path exists but is not a directory: {requested}")
    except Exception as e:  # noqa: BLE001
        raise RuntimeError(f"Failed to ensure local data root directory at {requested}") from e


# Ensure the data root exists so downloads always target this folder.
_local_data_root = _ensure_local_data_root(DS_RD_SETTING.local_data_path)
DS_RD_SETTING.local_data_path = str(_local_data_root)
...
# MLEBench prepared data can differ from Kaggle's required submission schema/row count.
assert not (
    DS_RD_SETTING.kaggle_live_eval and DS_RD_SETTING.if_using_mle_data
), "DS_KAGGLE_LIVE_EVAL and DS_IF_USING_MLE_DATA cannot both be true; disable MLE data mode for live Kaggle submissions"
