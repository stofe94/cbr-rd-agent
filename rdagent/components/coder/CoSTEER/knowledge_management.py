...
class CoSTEERRAGStrategy(RAGStrategy):
    def __init__(self, *args, dump_knowledge_base_path: Path = None, **kwargs):
        ...
        if dump_knowledge_base_path is None:
            dump_knowledge_base_path = os.environ.get("CoSTEER_NEW_KNOWLEDGE_BASE_PATH")
            if dump_knowledge_base_path:
                dump_knowledge_base_path = Path(dump_knowledge_base_path)
    ...
    def dump_knowledge_base(self):
        if self.dump_knowledge_base_path is None:
            logger.warning(
                "Dump knowledge base path is not set. Set CoSTEER_NEW_KNOWLEDGE_BASE_PATH env var to enable persistence."
            )
        else:
...
class CoSTEERKnowledgeBaseV2(EvolvingKnowledgeBase):
    def __init__(self, init_component_list=None, path: str | Path = None) -> None:
        ...
        """
        self.graph: UndirectedGraph = UndirectedGraph(get_safe_cwd() / "graph.pkl")
