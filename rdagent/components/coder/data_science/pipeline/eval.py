...
UNSUPPORTED_KWARG_RE = re.compile(
    r"TypeError:\s*(?P<func>[\w\.]+)\(\)\s*got an unexpected keyword argument\s*['\"](?P<arg>[^'\"]+)['\"]"
)


def _extract_unsupported_kwarg_findings(stdout: str) -> list[tuple[str, str]]:
    findings: list[tuple[str, str]] = []
    for match in UNSUPPORTED_KWARG_RE.finditer(stdout):
        findings.append((match.group("func"), match.group("arg")))
    # keep order and deduplicate
    return list(dict.fromkeys(findings))


def _extract_traceback_block(stdout: str) -> str:
    m = re.search(r"Traceback \(most recent call last\):[\s\S]*", stdout)
    return m.group(0).strip() if m else ""


def _extract_code_hints_for_kwargs(code: str, kwargs: list[str]) -> str:
    if not kwargs:
        return ""
...
def _build_api_compatibility_note(code: str, stdout: str) -> tuple[str, str]:
    findings = _extract_unsupported_kwarg_findings(stdout)
    if not findings:
        return "", ""

    kwargs = [arg for _, arg in findings]
    lines = [
        "### Deterministic API Compatibility Findings",
        "The runtime parser detected unsupported keyword arguments:",
    ]
    for func, arg in findings:
        lines.append(f"- function={func}, unsupported_keyword={arg}")
    lines.append("Required action: inspect runtime function signatures and only pass supported kwargs.")

    traceback_block = _extract_traceback_block(stdout)
    supplementary_code = _extract_code_hints_for_kwargs(code, kwargs)
    structured_error_message = (
        f"### TRACEBACK: {traceback_block if traceback_block else '[not found]'}\n"
        f"### SUPPLEMENTARY_INFO: unsupported_keywords={kwargs}"
    )
    if supplementary_code:
        structured_error_message += f"\nCode hints:\n{supplementary_code}"

    return "\n".join(lines), structured_error_message
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        if DS_RD_SETTING.sample_data_by_LLM:
            ...
            entry = (
                "test -f main.py || (echo 'MISSING main.py in container workdir:'; pwd; ls -la; exit 2); "
                "strace -e trace=file -f -o trace.log python -m coverage run main.py --debug"
            )
            result = implementation.run(env=env, entry=entry)
        else:
            entry = (
                "test -f main.py || (echo 'MISSING main.py in container workdir:'; pwd; ls -la; exit 2); "
                "strace -e trace=file -f -o trace.log python -m coverage run main.py"
            )
            result = implementation.run(env=env, entry=entry)
        result_stdout = result.stdout
        api_compat_note, structured_api_error_message = _build_api_compatibility_note(
            implementation.file_dict["main.py"], result_stdout
        )
        ...
        wfb = build_cls_from_json_with_retry(
            ...
            auto_fallback_to_prompt_json=True,
        )

        if structured_api_error_message:
            if not wfb.error_message:
                wfb.error_message = structured_api_error_message
            else:
                wfb.error_message += f"\n\n{structured_api_error_message}"
            if wfb.requires_documentation_search is None:
                wfb.requires_documentation_search = True

        # judge whether we should perform documentation search
        do_documentation_search = enable_mcp_documentation_search and (
            bool(wfb.requires_documentation_search) or bool(structured_api_error_message)
        )
