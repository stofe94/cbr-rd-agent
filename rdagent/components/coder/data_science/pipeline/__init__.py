...
class PipelineMultiProcessEvolvingStrategy(MultiProcessEvolvingStrategy):
    @staticmethod
    def _build_runtime_api_guardrails(package_info: str | dict | list | None) -> str:
        """Build deterministic API-compatibility instructions for code generation."""
        guardrails = [
            "## Runtime API Compatibility Guardrails",
            "- Do not assume latest library APIs.",
            "- Before calling library functions with optional kwargs, inspect runtime support with `inspect.signature` and only pass supported kwargs.",
            "- If a keyword is unsupported at runtime, remove it instead of retrying the same call pattern.",
        ]

        pkg_text = package_info if isinstance(package_info, str) else str(package_info or "")
        if re.search(r"xgboost\s*(==|>=|<=|~=)?\s*[0-9]", pkg_text, flags=re.IGNORECASE):
            guardrails.extend(
                [
                    "",
                    "### XGBoost-Specific Guardrail",
                    "- Build `fit_kwargs` dynamically from `inspect.signature(model.fit).parameters`.",
                    "- Only include keys that exist in the runtime signature (e.g., `eval_set`, `verbose`, `callbacks`, `early_stopping_rounds`, `eval_metric`).",
                    "- Never pass both legacy and new alternatives blindly; choose the supported one at runtime.",
                ]
            )
    ...
    def implement_one_task(
        ...
        system_prompt = T(".prompts:pipeline_coder.system").r(
            ...
            runtime_api_guardrails=self._build_runtime_api_guardrails(target_task.package_info),
