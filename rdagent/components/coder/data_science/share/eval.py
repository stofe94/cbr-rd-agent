...
class ModelDumpEvaluator(CoSTEEREvaluator):
    ...
    def evaluate(
        ...
        missing_artifacts_before_inference = (
            submission_content_before == NO_SUB or scores_content_before == NO_SCORE
        )

        # Remove the files submission.csv and scores.csv
        implementation.execute(
            env=env,
            entry=get_clear_ws_cmd(
                stage="before_inference",
                tolerate_missing=missing_artifacts_before_inference,
            ),
        )
