...
def workspace_win(workspace, cmp_workspace=None, cmp_name="last code."):
    ...
    if len(show_files) > 0:
        ...
        with st.popover(
            ...
            file_names = list(show_files.keys())
            code_tabs = st.tabs(file_names)
            for ct, codename in zip(code_tabs, file_names):
                with ct:
                    st.code(
                        show_files[codename],
...
def llm_log_win(llm_d: list):
    ...
    for d in llm_d:
        if "debug_tpl" in d["tag"]:
            ...
            cxt = d["obj"]["context"]
            rd = d["obj"]["rendered"]
            with st.popover(highlight_prompts_uri(uri), width="stretch", icon="⚙️"):
            ...
            with st.expander(f"**LLM** {time_info_str}", icon="🤖", expanded=False):
                ...
                with t4:
                    ...
                    with input_c:
                        ...
                        trace_model = state.data.get("settings", {}).get("LITELLM_SETTINGS", {}).get("chat_model")
                        trace_reasoning_effort = (
                            state.data.get("settings", {}).get("LITELLM_SETTINGS", {}).get("reasoning_effort", None)
                        )
                        if available_models:
                            if trace_model not in available_models:
                                trace_model = available_models[0]
                            ...
                            fallback_model = btc1.text_input(
                                "Chat Model",
                                value=trace_model or "",
                                key=key + "_chat_model_fallback",
                            )
                            LITELLM_SETTINGS.chat_model = fallback_model.strip() or None
...
def running_win(data, base_exp, llm_data=None, last_sota_exp=None):
    ...
    if "no_tag" in data:
        ...
        submission_score_text = data.get("submission_score", "no submission to score")
        submission_score = extract_json(submission_score_text)
        st.subheader(
            "Submission Score"
            + ("✅" if (isinstance(submission_score, dict) and submission_score["score"] is not None) else "❌")
        )
        if isinstance(submission_score, dict):
            st.json(submission_score)
        else:
            st.code(submission_score_text, wrap_lines=True)
...
def summarize_win():
    ...
    with st.container(border=True):
        ...
        info0, info1, info2, info3, info4, info5, info6, info7, info8 = st.columns(9)
        show_trace_dag = info0.toggle("Show trace DAG", key="show_trace_dag")
        only_success = info0.toggle("Only Success", key="only_success")
        litellm_settings = state.data.get("settings", {}).get("LITELLM_SETTINGS")
        if litellm_settings:
            with info1.popover("LITELLM", icon="⚙️"):
                st.write(litellm_settings)
        with info2.popover("GOOGLE_AI_STUDIO", icon="⚙️"):
            st.write(state.data.get("settings", {}).get("GOOGLE_AI_STUDIO_SETTINGS", "No settings found."))
        with info3.popover("RD_AGENT", icon="⚙️"):
            st.write(state.data.get("settings", {}).get("RD_AGENT_SETTINGS", "No settings found."))
        with info4.popover("RDLOOP", icon="⚙️"):
        ...
        info5.metric("LLM Calls", llm_call, help=timedelta_to_str(llm_call_duration))
        info6.metric(
            "LLM Filter Calls",
            llm_filter_call,
        ...
        trace_msgs = list(FileStorage(state.log_folder / state.log_path).iter_msg(tag="record.trace"))
        if not trace_msgs:
            st.warning(
                "No record.trace data found for this session yet. "
                "The run may have stopped before trace recording completed."
            )
            return
        final_trace = trace_msgs[-1].content
        if show_trace_dag:
        ...
        for loop in range(min_id, max_id + 1):
            ...
            if "running" in loop_data and "no_tag" in loop_data["running"]:
                try:
                    ...
                    if not isinstance(running_result, pd.DataFrame) or running_result.empty:
                        raise ValueError("running result is empty")
                    if "ensemble" not in running_result.index:
                        raise KeyError("missing 'ensemble' row in running result")
                    if running_result.shape[1] == 0:
                        raise ValueError("running result has no metric columns")
                    ...
                    valid_result_issues.append(f"Loop {loop}: {type(e).__name__}: {e}")
                if "submission_score" not in state.data[loop]:
                    if "submission_score" in loop_data["running"]:
                        submission_score_txt = loop_data["running"]["submission_score"]
                        state.data[loop]["submission_score"] = extract_json(submission_score_txt)
                        if (
                            state.data[loop]["submission_score"] is not None
                            and state.data[loop]["submission_score"]["score"] is not None
                        ):
                            medal_emoji = (
                                "🥇"
                                if state.data[loop]["submission_score"]["gold_medal"]
                                else (
                                    "🥈"
                                    if state.data[loop]["submission_score"]["silver_medal"]
                                    else "🥉" if state.data[loop]["submission_score"]["bronze_medal"] else ""
                                )
                            )
                            df.loc[loop, "Run Score (test)"] = f"{medal_emoji} {state.data[loop]['submission_score']['score']}"
                        else:
                            state.data[loop]["submission_score"] = submission_score_txt
                            df.loc[loop, "Run Score (test)"] = "❌"
                    else:
                        ws_path = replace_ep_path(loop_data["running"]["no_tag"].experiment_workspace.workspace_path)
                        submission_score_path = ws_path / "submission_score.txt"
                        try:
                            submission_score_txt = submission_score_path.read_text()
                            state.data[loop]["submission_score"] = extract_json(submission_score_txt)
                            if state.data[loop]["submission_score"]["score"] is not None:
                                medal_emoji = (
                                    "🥇"
                                    if state.data[loop]["submission_score"]["gold_medal"]
                                    else (
                                ...
                                df.loc[loop, "Run Score (test)"] = (
                                    f"{medal_emoji} {state.data[loop]['submission_score']['score']}"
                                )
                            else:
                                state.data[loop]["submission_score"] = submission_score_txt
                                df.loc[loop, "Run Score (test)"] = "❌"
                        except Exception as e:
                            state.data[loop]["submission_score"] = str(e)
                            df.loc[loop, "Run Score (test)"] = "❌"
                else:
                    if isinstance(state.data[loop]["submission_score"], dict):
                        medal_emoji = (
                            "🥇"
                            if state.data[loop]["submission_score"]["gold_medal"]
                            else (
        ...
        # timeline figure
        if state.times:
            with st.popover("Timeline", width="stretch", icon="⏱️"):
        ...
        if valid_result_issues:
            st.warning(
                f"Skipped valid-score extraction for {len(valid_result_issues)} loop(s). Expand details below to inspect errors."
            )
            with st.expander("Valid Score Extraction Issues", expanded=False):
                for issue in valid_result_issues:
                    st.write(f"- {issue}")
        if len(vscores) == 0:
            st.warning("No usable valid-score data found. Skipping scores curve.")
        else:
            ...
            with st.popover("Scores Curve", width="stretch", icon="📈"):
        ...
        with st2.popover("Component Statistics", width="stretch", icon="📊"):
        ...
        with st1.popover("Time Statistics", width="stretch", icon="⏱️"):
        ...
        with st.popover("COST Curve", width="stretch", icon="💰"):
            fig = px.line(
                x=costs.index,
