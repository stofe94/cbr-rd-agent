...
if not Path(aide_path).exists():
    st.warning(f"Path {aide_path} does not exist. Set `UI_AIDE_PATH` to a directory containing AIDE traces.")
    st.stop()

jps = [str(i) for i in Path(aide_path).rglob("**/filtered_journal.json")]
jps = sorted(jps)
if not jps:
    st.info(f"No filtered_journal.json files were found under {aide_path}. Set `UI_AIDE_PATH` to a valid AIDE export directory.")
    st.stop()
# st.write(jps)
