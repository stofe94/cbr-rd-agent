...
def _find_loop_score_pickles(log_path: Path, loop_id: int) -> list[Path]:
    return [i for i in log_path.rglob(f"Loop_{loop_id}/running/submission_score/**/*.pkl")]
...
class ValidationSelector(SOTAexpSelector):
    ...
    def print_code(self, data_py_code: str, grade_py_code: str):
        ...
        print("\033[1;30;106m======== data.py ========\033[0m")
        print(data_py_code)
        print("\033[1;30;106m======== grade.py ========\033[0m")
        print(grade_py_code)
        print("\033[1;30;106m======== code end ========\033[0m")
...
def evaluate_one_trace(
    ...
    if selector_name == "validation":
        ...
        if debug:
            ...
            for exp in candidate_exps:
                ...
                sota_submission_score_paths = _find_loop_score_pickles(log_path, loop_id)
                if len(sota_submission_score_paths):
                    with sota_submission_score_paths[0].open("rb") as f:
                        sota_submission_score = extract_json(pickle.load(f))
                        if sota_submission_score.get("any_medal", False):
                            pool_hit = True
                            break
        if not pool_hit:
    ...
    if debug:
        ...
        if len(sota_submission_score_paths):
            with sota_submission_score_paths[0].open("rb") as f:
                sota_submission_score = extract_json(pickle.load(f))
                hit = sota_submission_score.get("any_medal", False)
                if hit:
                    if sota_submission_score["gold_medal"]:
                        sota_exp_stat = "gold"
                    elif sota_submission_score["silver_medal"]:
                        sota_exp_stat = "silver"
                    elif sota_submission_score["bronze_medal"]:
