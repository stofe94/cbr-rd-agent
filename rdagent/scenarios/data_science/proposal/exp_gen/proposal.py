...
_ML_RELEVANT_PACKAGES: list[str] = [
    "numpy",
    "pandas",
    "scikit-learn",
    "scipy",
        ...
        if component_info:
            ...
            system_prompt = T(".prompts:direct_exp_gen.system").r(
                ...
                hypothesis_specification=(
                    (T(".prompts:hypothesis_specification").r())
                    + ("\n\nCBR Retrieved Cases:\n" + trace.cbr_retrieved_for_planning if hasattr(trace, "cbr_retrieved_for_planning") and trace.cbr_retrieved_for_planning else "")
                ),
...
class DSProposalV2ExpGen(ExpGen):
    ...
    @staticmethod
    def _normalize_challenge_response(payload: Any) -> Dict[str, Dict[str, str]]:
        """Normalize LLM challenge outputs from either schema object or top-level list."""
        if isinstance(payload, dict):
            challenge_items = payload.get("challenges")
            if isinstance(challenge_items, list):
                payload = challenge_items
            else:
                return payload

        if not isinstance(payload, list):
            raise TypeError(f"Unexpected challenge payload type: {type(payload).__name__}")

        normalized: Dict[str, Dict[str, str]] = {}
        for i, item in enumerate(payload, start=1):
            if not isinstance(item, dict):
                continue
            caption = str(item.get("caption") or item.get("challenge_name") or f"challenge_{i}")
            problem = str(item.get("statement") or item.get("description") or item.get("problem") or "")
            reason = str(item.get("reasoning") or item.get("reason") or item.get("metric_impact") or "")
            normalized[caption] = {"problem": problem, "reason": reason}

        if not normalized:
            raise ValueError("Challenge payload list is empty or malformed")
        return normalized

    @staticmethod
    def _normalize_hypothesis_response(payload: Any) -> Dict[str, Dict[str, Any]]:
        """Normalize hypothesis payloads from strict schema or simplified JSON structures."""
        if isinstance(payload, dict) and isinstance(payload.get("hypotheses"), list):
            hypothesis_items = payload["hypotheses"]
        elif isinstance(payload, list):
            hypothesis_items = payload
        else:
            raise TypeError(f"Unexpected hypothesis payload type: {type(payload).__name__}")

        valid_components = {c.value for c in HypothesisComponent}
        normalized: Dict[str, Dict[str, Any]] = {}

        for i, item in enumerate(hypothesis_items, start=1):
            if not isinstance(item, dict):
                continue

            caption = str(item.get("caption") or item.get("challenge") or f"hypothesis_{i}")
            reason = str(item.get("challenge") or item.get("metric_impact") or "")
            hypothesis = str(item.get("hypothesis") or "")

            component = str(item.get("component") or item.get("component_tag") or HypothesisComponent.FeatureEng.value)
            if component not in valid_components:
                component = HypothesisComponent.FeatureEng.value

            evaluation = item.get("evaluation") if isinstance(item.get("evaluation"), dict) else {}

            def _score(name: str, fallback: str) -> float:
                score_value = evaluation.get(name, {}) if isinstance(evaluation, dict) else {}
                if isinstance(score_value, dict):
                    score_value = score_value.get("score")
                if score_value is None:
                    score_value = item.get(fallback)
                try:
            ...
            normalized[caption] = {
                ...
                "evaluation": {
                    "alignment_score": _score("alignment", "challenge_hypothesis_alignment"),
                    "impact_score": _score("impact", "expected_impact"),
                    "novelty_score": _score("novelty", "novelty"),
                    "feasibility_score": _score("feasibility", "feasibility"),
                    "risk_reward_balance_score": _score("risk_reward_balance", "risk_reward_balance"),
                },
            }

        if not normalized:
            raise ValueError("Hypothesis payload is empty or malformed")
        return normalized
    ...
    def identify_scenario_problem(
        ...
        if self.supports_response_schema:
            ...
            problems = self._normalize_challenge_response(response_payload)
...
You help users retrieve relevant knowledge from community discussions and public code.""")
        ...
        if self.supports_response_schema:
            response_payload = json.loads(response)
            resp_dict = self._normalize_hypothesis_response(response_payload)
        else:
    ...
    def task_gen(
        ...
        # Always provide concise ML-relevant package versions.
        # If the LLM provides additional package names, extend this list with a capped size.
        requested_pkgs: list[str] = []
        if isinstance(task_dict, dict) and "packages" in task_dict and isinstance(task_dict["packages"], list):
            requested_pkgs = [str(p) for p in task_dict["packages"]]

        merged_pkgs = list(dict.fromkeys(_ML_RELEVANT_PACKAGES + requested_pkgs))[:20]
        task.package_info = get_packages(merged_pkgs)
    ...
    def gen(
        ...
        all_problems = self.identify_problem(
            ...
            sibling_exp=sibling_exp,
            cbr_context=getattr(trace, "cbr_retrieved_for_planning", None),
        )
