...
def _pipeline_step_banner(step: str, message: str, color: str) -> str:
    fg_to_bg = {
        LogColors.BLUE: "\033[104m",    # bright blue
        LogColors.CYAN: "\033[106m",    # bright cyan
        LogColors.GREEN: "\033[102m",   # bright green
        LogColors.YELLOW: "\033[103m",  # bright yellow
        LogColors.MAGENTA: "\033[105m", # bright magenta
        LogColors.RED: "\033[101m",     # bright red
    }
    bg = fg_to_bg.get(color, "\033[106m")
    # Bold black text on bright background
    return f"\033[1;30m{bg}=== PIPELINE STEP: {step} | {message} ===\033[0m"
...
def cleanup_irrelevant_sub_workspaces(exp: DSExperiment) -> None:
    """Delete helper sub-workspaces that are not meant to be persisted."""
    for ws in exp.sub_workspace_list:
        if ws is None:
            continue
        if ws is exp.experiment_workspace:
            continue
        if ws.target_task is None:
            ws.clear()
...
class DataScienceRDLoop(RDLoop):
    ...
    async def direct_exp_gen(self, prev_out: dict[str, Any]):
        logger.info(
            _pipeline_step_banner(
                "direct_exp_gen",
                "Targeting model for hypothesis/experiment generation (drafting, selection)",
                LogColors.CYAN,
            )
        )
    ...
    def coding(self, prev_out: dict[str, Any]):
        logger.info(_pipeline_step_banner("coding", "Targeting model for code generation", LogColors.MAGENTA))
        exp = prev_out["direct_exp_gen"]
        for tasks in exp.pending_tasks_list:
            exp.sub_tasks = tasks
            task_name = exp.sub_tasks[0].__class__.__name__
            logger.info(
                _pipeline_step_banner(
                    f"coding.{task_name}",
                    f"Generating code for {task_name}",
                    LogColors.MAGENTA,
                )
            )
    ...
    def running(self, prev_out: dict[str, Any]):
        logger.info(
            _pipeline_step_banner(
                "running",
                "Executing generated code (no model targeting in this step)",
                LogColors.GREEN,
            )
        )
        ...
        if exp.is_ready_to_run():
            logger.info(
                _pipeline_step_banner(
                    "running",
                    f"Executing experiment for {exp.hypothesis.component}",
                    LogColors.GREEN,
                )
            )
    ...
    def feedback(self, prev_out: dict[str, Any]) -> ExperimentFeedback:
        ...
        logger.info(
            _pipeline_step_banner(
                "feedback",
                "Targeting model for experiment evaluation and feedback generation",
                LogColors.YELLOW,
            )
        )
        ...
        if self.trace.next_incomplete_component() is None or DS_RD_SETTING.coder_on_whole_pipeline:
            ...
            logger.info(
                _pipeline_step_banner(
                    ...
                    "Generating comprehensive feedback using model",
                    LogColors.YELLOW,
                )
            )
            ...
            logger.info(
                _pipeline_step_banner(
                    "feedback.simple",
                    f"Draft completed for {exp.hypothesis.component}, using simple feedback (no model)",
                    LogColors.YELLOW,
                )
            )
    ...
    def record(self, prev_out: dict[str, Any]):
        ...
        if e is None:
            ...
            cleanup_irrelevant_sub_workspaces(exp)
        else:
    ...
    def load(
        ...
        logger.log_object(DS_RD_SETTING.competition, tag="competition")  # NOTE: necessary to make submission summary work.
        if DS_RD_SETTING.enable_knowledge_base and DS_RD_SETTING.knowledge_base_version == "v1":
